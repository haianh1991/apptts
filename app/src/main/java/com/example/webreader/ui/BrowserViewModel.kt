...
class BrowserViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val translationChannel = kotlinx.coroutines.channels.Channel<TranslationJobRequest>(kotlinx.coroutines.channels.Channel.UNLIMITED)
    ...
    init {
        ...
        viewModelScope.launch(Dispatchers.Default) {
            for (request in translationChannel) {
    ...
    fun translateWebpage(text: String, title: String, url: String, folderId: String? = null) {
        ...
        viewModelScope.launch {
            ...
            _currentParagraphIndex.value = -1
            _foregroundTranslationStep.value = "Đang bắt đầu dịch thuật..."
            _foregroundTranslationSteps.value = listOf("Đang bắt đầu dịch thuật...")
            ...
            result.onSuccess { translatedText ->
                val (extractedTitle, contentText) = parseTitleAndContent(translatedText, title)
                currentTitle = extractedTitle
    ...
    fun translateAndAddToQueue(text: String, title: String, url: String, folderId: String? = null) {
        ...
        val job = ActiveTranslation(
            ...
            currentStep = "Đang xếp hàng chờ dịch..."
        )
        _activeTranslations.value = _activeTranslations.value + job

        android.widget.Toast.makeText(
            getApplication(),
            "Đã thêm vào hàng xếp dịch: $title",
        ...
        translationChannel.trySend(jobRequest)
    }
    ...
    private suspend fun processTranslationRequest(request: TranslationJobRequest) {
        ...
        val logSteps = request.logSteps

        // Cập nhật trạng thái sang TRANSLATING khi bắt đầu dịch thực sự
        _activeTranslations.value = _activeTranslations.value.map {
            if (it.id == logId) it.copy(status = TranslationStatus.TRANSLATING, currentStep = "Bắt đầu dịch thực tế...") else it
        }
    ...
    fun retryTranslation(job: ActiveTranslation) {
        ...
        viewModelScope.launch {
            ...
            result.onSuccess { translatedText ->
                val (extractedTitle, contentText) = parseTitleAndContent(translatedText, job.title)
                currentTitle = extractedTitle
    ...
    private fun parseTitleAndContent(text: String, originalTitle: String): Pair<String, String> {
        val trimmed = text.trimStart()
        if (trimmed.startsWith("Title:", ignoreCase = true)) {
            val titleLineEnd = trimmed.indexOf('\n')
            if (titleLineEnd != -1) {
                val titleLine = trimmed.substring(0, titleLineEnd)
                val extractedTitle = titleLine.substring(6).trim()
                val content = trimmed.substring(titleLineEnd).trim()
                val finalTitle = if (extractedTitle.isNotBlank()) extractedTitle else originalTitle
                return Pair(finalTitle, content)
            } else {
                val extractedTitle = trimmed.substring(6).trim()
                val finalTitle = if (extractedTitle.isNotBlank()) extractedTitle else originalTitle
                return Pair(finalTitle, "")
            }
        } else {
            val prefix = "title:"
            if (prefix.startsWith(trimmed.lowercase())) {
                return Pair(originalTitle, "")
            }
            return Pair(originalTitle, text)
        }
    }
}
...
data class TranslationJobRequest(
    ...
    val url: String,
    val folderId: String?,
    val logSteps: MutableList<String>
)
